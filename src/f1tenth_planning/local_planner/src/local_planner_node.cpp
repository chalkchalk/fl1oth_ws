...
using f1tenth_common::NodeState;
LocalPlannerNode::LocalPlannerNode() :
    local_planner_nh_("~"),
    as_(local_planner_nh_, "/local_planner_node_action", boost::bind(&LocalPlannerNode::ExcuteCB, this, _1), false),
    initialized_(false), node_state_(f1tenth_common::NodeState::IDLE),
    node_error_info_(f1tenth_common::ErrorCode::OK), max_error_(5),
...
f1tenth_common::ErrorInfo LocalPlannerNode::Init() {
  ROS_INFO("local planner start");
  LocalAlgorithms local_algorithms;
  std::string full_path = ros::package::getPath("f1tenth_planning") + "/local_planner/config/local_planner.prototxt";
  f1tenth_common::ReadProtoFromTextFile(full_path.c_str(), &local_algorithms);
  if (&local_algorithms == nullptr) {
    return f1tenth_common::ErrorInfo(f1tenth_common::ErrorCode::LP_INITILIZATION_ERROR,
  ...
  std::string map_path = ros::package::getPath("f1tenth_costmap") + \
      "/config/costmap_parameter_config_for_local_plan.prototxt";
  local_cost_ = std::make_shared<f1tenth_costmap::CostmapInterface>("local_costmap",
                                                                          *tf_,
                                                                          map_path.c_str());
  local_planner_ = f1tenth_common::AlgorithmFactory<LocalPlannerBase>::CreateAlgorithm(selected_algorithm_);
  ...
  vel_pub_ = local_planner_nh_.advertise<f1tenth_msgs::TwistAccel>("/cmd_vel_raw", 5);

  return f1tenth_common::ErrorInfo(f1tenth_common::ErrorCode::OK);
}

void LocalPlannerNode::ExcuteCB(const f1tenth_msgs::LocalPlannerGoal::ConstPtr &command) {
  f1tenth_common::ErrorInfo error_info = GetErrorInfo();
  NodeState node_state = GetNodeState();

  if (node_state == NodeState::FAILURE) {
    f1tenth_msgs::LocalPlannerFeedback feedback;
    f1tenth_msgs::LocalPlannerResult result;
  ...
  while (ros::ok()) {
    ...
    if (node_state == NodeState::RUNNING|| node_state == NodeState::SUCCESS
      ...
      if (!error_info.IsOK()) {
        feedback.error_code = error_info.error_code();
        feedback.error_msg = error_info.error_msg();
        SetErrorInfo(f1tenth_common::ErrorInfo::OK());
...
void LocalPlannerNode::Loop() {
  ...
  f1tenth_common::ErrorInfo error_info = local_planner_->Initialize(local_cost_, tf_, visual_);
  if (error_info.IsOK()) {
  ...
  while (GetNodeState() == NodeState::RUNNING) {
    ...
    f1tenth_common::ErrorInfo error_info = local_planner_->ComputeVelocityCommands(cmd_vel_);
    ...
    if (error_info.IsOK()) {
      ...
      error_info =  f1tenth_common::ErrorInfo(f1tenth_common::ErrorCode::LP_MAX_ERROR_FAILURE, "over max error.");
      SetNodeState(NodeState::FAILURE);
    } else {
      error_count++;
...
void LocalPlannerNode::SetErrorInfo(const f1tenth_common::ErrorInfo error_info) {
  std::lock_guard<std::mutex> guard(node_error_info_mtx_);
  node_error_info_ = error_info;
}

void LocalPlannerNode::SetNodeState(const f1tenth_common::NodeState& node_state) {
  std::lock_guard<std::mutex> guard(node_state_mtx_);
  node_state_ = node_state;
}

f1tenth_common::NodeState LocalPlannerNode::GetNodeState() {
  std::lock_guard<std::mutex> guard(node_state_mtx_);
  return node_state_;
}

f1tenth_common::ErrorInfo LocalPlannerNode::GetErrorInfo() {
  std::lock_guard<std::mutex> guard(node_error_info_mtx_);
...
void LocalPlannerNode::StartPlanning() {
  ...
  SetNodeState(f1tenth_common::NodeState::RUNNING);
  local_planner_thread_= std::thread(std::bind(&LocalPlannerNode::Loop,this));
}

void LocalPlannerNode::StopPlanning() {
  SetNodeState(f1tenth_common::IDLE);
  if (local_planner_thread_.joinable()) {
    local_planner_thread_.join();
  }
}

void LocalPlannerNode::AlgorithmCB(const f1tenth_common::ErrorInfo &algorithm_error_info) {
...
int main(int argc, char **argv) {
  ...
  f1tenth_local_planner::LocalPlannerNode local_planner;
