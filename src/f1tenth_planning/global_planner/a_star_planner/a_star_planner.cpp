...
AStarPlanner::AStarPlanner(CostmapPtr costmap_ptr) :
  ...
  std::string full_path = ros::package::getPath("f1tenth_planning") + "/global_planner/a_star_planner/"\
...
ErrorInfo AStarPlanner::Plan(const geometry_msgs::PoseStamped &start,
  ...
  if (!goal_valid){
    ...
    costmap_ptr_->GetCostMap()->SetCost(start_x, start_y,f1tenth_costmap::FREE_SPACE);
