...
class VelConverter {
 public:
  VelConverter() : new_cmd_acc_(false), begin_(false) {
    ...
    cmd_vel_acm_.drive.steering_angle = 0;
    cmd_vel_acm_.drive.speed = 0;

    //cmd_pub_ = cmd_handle_.advertise<geometry_msgs::Twist>("/cmd_vel", 5);
    cmd_pub_acm_ = cmd_handle_.advertise<ackermann_msgs::AckermannDriveStamped>("/cmd_vel", 5);
    cmd_sub_ = cmd_handle_.subscribe<f1tenth_msgs::TwistAccel>("/cmd_vel_raw", 100, boost::bind(&VelConverter::VelCallback, this, _1));
  }
  void VelCallback(const f1tenth_msgs::TwistAccel::ConstPtr& msg);
  void UpdateVel();

 private:
  f1tenth_msgs::TwistAccel cmd_vel_acc_;
  geometry_msgs::Twist cmd_vel_;
  ackermann_msgs::AckermannDriveStamped cmd_vel_acm_;
  bool new_cmd_acc_, begin_;

  ros::NodeHandle cmd_handle_;
  //ros::Publisher cmd_pub_;
  ros::Publisher cmd_pub_acm_;
...
void VelConverter::VelCallback(const f1tenth_msgs::TwistAccel::ConstPtr& twist_acc_msg)
{
  if (!begin_) {
    begin_ = true;
  }
...
void VelConverter::UpdateVel() {
  ...
  if (new_cmd_acc_) {
    ...
    cmd_vel_acm_.drive.speed = cmd_vel_acc_.twist.linear.x;
    cmd_vel_acm_.drive.steering_angle = cmd_vel_acc_.twist.angular.z;
    cmd_pub_acm_.publish(cmd_vel_acm_);
    new_cmd_acc_ = false;
