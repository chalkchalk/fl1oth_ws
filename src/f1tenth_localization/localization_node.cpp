...
bool LocalizationNode::Init() {
  ...
  laser_scan_filter_ = std::make_unique<tf::MessageFilter<sensor_msgs::LaserScan>>(*laser_scan_sub_,
                                                                                   *tf_listener_ptr_,
                                                                                   odom_frame_,
                                                                                   100);
  ROS_ERROR("odom:%s",odom_frame_.c_str());
  ROS_ERROR("set clbk");
...
int main(int argc, char **argv) {
  f1tenth_localization::GLogWrapper glog_wrapper(argv[0]);
  ros::init(argc, argv, "localization_node");
  f1tenth_localization::LocalizationNode localization_node("localization_node");
