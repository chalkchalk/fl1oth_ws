...
#ifndef f1tenth_COMMON_MATH_H
#define f1tenth_COMMON_MATH_H
...
inline double Cross2D (const f1tenth_common::Point2D &vector0,
                       const f1tenth_common::Point2D &vector1) {
  return vector0.X() * vector1.Y() - vector0.Y() * vector1.X();
}

inline double Cross2D (const f1tenth_common::Point2D &start_point,
                       const f1tenth_common::Point2D &point0,
                       const f1tenth_common::Point2D &point1) {
...
inline bool ConvexVerify(std::vector<f1tenth_common::Point2D> &points) {
...
inline double PointDistance (const f1tenth_common::Point2D &point0,
                             const f1tenth_common::Point2D &point1) {
...
inline double PointToLineDistance (const f1tenth_common::Point2D &point,
                                   const f1tenth_common::LineSegment2D &line) {
...
inline bool CheckLineSegmentsIntersection2D(const f1tenth_common::LineSegment2D &line0,
                                            const f1tenth_common::LineSegment2D &line1,
                                            f1tenth_common::Point2D *intersection = NULL) {
  double s_numer, t_numer, denom, t;
  ...
  f1tenth_common::LineSegment2D aux(line1.Start(), line0.Start());
  ...
  t = t_numer / denom;
  if (intersection) {
    *intersection = line0.Start() + f1tenth_common::Point2D(t * line0.DiffX(), t * line0.DiffY());
  }
...
inline double DistanceSegmentToSegment2D(const f1tenth_common::LineSegment2D &line0, const f1tenth_common::LineSegment2D &line1) {
...
inline double DistancePointToPolygon2D(const f1tenth_common::Point2D &point, const f1tenth_common::Polygon2D &polygon) {
  double dist = HUGE_VAL;
...
inline double DistanceSegmentToPolygon2D(const f1tenth_common::LineSegment2D line,
                                         const f1tenth_common::Polygon2D polygon) {
  double dist = HUGE_VAL;
...
inline double DistancePolygonToPolygon2D(const f1tenth_common::Polygon2D &polygon0,
                                         const f1tenth_common::Polygon2D &polygon1) {
  double dist = HUGE_VAL;
...
inline f1tenth_common::Point2D PointRotateAroundPoint(const f1tenth_common::Point2D source,
                                                   const f1tenth_common::Point2D around,
                                                   double theta)
{
    f1tenth_common::Point2D val((source.X() - around.X())*std::cos(theta) - (source.Y() - around.Y())*std::sin(theta) + around.X(),
...
inline int PointInRect(const f1tenth_common::Point2D point,
                       const f1tenth_common::Point2D rect_1,
                       const f1tenth_common::Point2D rect_2)
{
...
{
  while(angle > f1tenth_common::PI)
    angle -= 2*f1tenth_common::PI;
  while(angle <= -f1tenth_common::PI)
    angle += 2*f1tenth_common::PI;
  return angle;
}
