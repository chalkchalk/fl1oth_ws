...
using f1tenth_common::ErrorCode;
using f1tenth_common::ErrorInfo;
...
ErrorInfo GlobalPlannerNode::Init() {
  ...
  std::string full_path = ros::package::getPath("f1tenth_planning") + "/global_planner/config/global_planner_config.prototxt";
  if (!f1tenth_common::ReadProtoFromTextFile(full_path.c_str(),
  ...
  costmap_ptr_ = std::make_shared<f1tenth_costmap::CostmapInterface>("global_costmap",
                                                                           *tf_ptr_,
                                                                           map_path.c_str());
  // Create the instance of the selected algorithm
  global_planner_ptr_ = f1tenth_common::AlgorithmFactory<GlobalPlannerBase,CostmapPtr >::CreateAlgorithm(
...
void GlobalPlannerNode::GoalCallback(const f1tenth_msgs::GlobalPlannerGoal::ConstPtr &msg) {
  ...
  while (ros::ok()) {
    ...
    if(node_state == NodeState::RUNNING || node_state == NodeState::SUCCESS || node_state == NodeState::FAILURE) {
      f1tenth_msgs::GlobalPlannerFeedback feedback;
      f1tenth_msgs::GlobalPlannerResult result;
...
void GlobalPlannerNode::PlanThread() {
  ...
  while (ros::ok()) {
    ...
    {
      std::unique_lock<f1tenth_costmap::Costmap2D::mutex_t> lock(*(costmap_ptr_->GetCostMap()->GetMutex()));
...
int main(int argc, char **argv) {
  ...
  f1tenth_global_planner::GlobalPlannerNode global_planner;
  ros::spin();
  return 0;
}
