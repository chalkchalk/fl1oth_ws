...
class GlobalPlannerTest{
 public:
  ...
  void DoneCallback(const actionlib::SimpleClientGoalState& state,  const f1tenth_msgs::GlobalPlannerResultConstPtr& result){
    ROS_INFO("The goal is done with %s!",state.toString().c_str());
  }
  void ActiveCallback() {
    ROS_INFO("Action server has recived the goal, the goal is active!");
  }
  void FeedbackCallback(const f1tenth_msgs::GlobalPlannerFeedbackConstPtr& feedback){
 ...
 private:
  ...
  f1tenth_msgs::GlobalPlannerGoal command_;
  actionlib::SimpleActionClient<f1tenth_msgs::GlobalPlannerAction> global_planner_actionlib_client_;
};
